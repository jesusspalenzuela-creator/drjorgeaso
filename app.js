...
async function cargarCitasDelServidor() {
    ...
    // Extraer campos únicos para las columnas dinámicas
    ...
    // Pintar tabla
    let cabeceras = ['ID', 'ID. Cliente', 'Paciente', 'Edad', 'Teléfono', 'F. Cita', 'H. Cita', 'Profesional', 'Acciones'];
    ...
    document.getElementById('tabla-cuerpo').innerHTML = citas.map(c => `
        <tr class="border-b">
            ...
            <td class="p-4"><button onclick="prepararEdicion(${JSON.stringify(c).replace(/"/g, '&quot;')})" class="text-blue-600 font-bold hover:underline">Editar</button></td>
...
function prepararEdicion(c) {
    idCitaEnEdicion = c.id; // Activamos el modo edición
...
document.getElementById('form-cita').addEventListener('submit', async (e) => {
    ...
    document.querySelectorAll('.nombre-campo').forEach((i, idx) => camposPersonalizadosObj[i.value.trim()] = document.querySelectorAll('.valor-campo')[idx].value);

    const payload = {
        ...(idCitaEnEdicion && { id: idCitaEnEdicion }), // Si hay ID, se añade al JSON
